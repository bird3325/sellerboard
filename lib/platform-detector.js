...
class PlatformDetector {
    ...
    static detect(url = window.location.href) {
        for (const [platform, patterns] of Object.entries(this.PATTERNS)) {
            for (const pattern of patterns) {
                if (pattern.test(url)) {
                    console.log(`[PlatformDetector] Detected platform: ${platform}`);
                    return platform;
                }
            }
        }

        console.log('[PlatformDetector] No specific platform detected, using generic parser');
    ...
    static async shouldShowWidget(url = window.location.href, userSettings = null) {
        ...
        console.log(`[PlatformDetector] Should show widget on ${platform}: ${shouldShow}`);
        return shouldShow;
    }
